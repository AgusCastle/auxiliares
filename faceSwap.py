...
"""
Presentacion:   1 slide el nombre del metodo para el caso de faceSwap con pasos mas significativos.
                2 slide lo de animacion
                * Con tiempos de que se tardan en la ejecucion.
                6 videos
                    Carlos 
                    Laura
                    Carmen
                    ...
                    Doc. Gab
                    Doc. Jesus
                3 face swap (3 cuerpos diferentes *los que se vean mejor)
                Cuerpo peña -> 
...
frontal_face_predictor = dlib.shape_predictor(
    "/Users/agustincastillo/Documents/Repositorios/auxiliares/shape_predictor_68_face_landmarks.dat")

# read the source face image and convert it to grayscale
source_image = cv2.imread(
    "./docs/laura.png")
#source_image = cv2.resize(source_image, (300, 300))
...
destination_image = cv2.imread(
    "./fusion/frida.jpeg")
#destination_image = cv2.resize(destination_image, (300, 300))
...
#############################################
    ...
    for triangle in triangles_array:
        ...
        # cv2.imshow(
        #     "5: Drawing all Delaunay triangles in source image ", source_face_image)

        # convert the co-ordinates into facial landmark references
        index_point1 = np.where(
            (source_face_landmark_points_array == index_point1).all(axis=1))
        index_point1 = index_from_array(index_point1)
        index_point2 = np.where(
            (source_face_landmark_points_array == index_point2).all(axis=1))
        index_point2 = index_from_array(index_point2)
        index_point3 = np.where(
            (source_face_landmark_points_array == index_point3).all(axis=1))
...
for destination_face in destination_faces:
    ...
    for landmark_no in range(0, 68):
        ...
        #             y_point), cv2.FONT_HERSHEY_SIMPLEX, .3, (255, 255, 255))
...
for i, triangle_index_points in enumerate(source_triangle_index_points_list):
    ...
    source_triangle = np.array(
        [source_triangle_point1, source_triangle_point2, source_triangle_point3], np.int32)
    ...
    source_triangle_points = np.array([[source_triangle_point1[0] - x, source_triangle_point1[1] - y],
                                       [source_triangle_point2[0] - x,
                                           source_triangle_point2[1] - y],
    ...
    # cv2.line(source_image, source_triangle_point1,
    #          source_triangle_point2, (255, 255, 255))
    # cv2.line(source_image, source_triangle_point2,
    #          source_triangle_point3, (255, 255, 255))
    # cv2.line(source_image, source_triangle_point3,
    #          source_triangle_point1, (255, 255, 255))
    # if i == 40:
    ...
    destination_triangle = np.array(
        [destination_triangle_point1, destination_triangle_point2, destination_triangle_point3], np.int32)
    ...
    destination_triangle_points = np.array([[destination_triangle_point1[0] - x, destination_triangle_point1[1] - y],
                                            [destination_triangle_point2[0] - x,
                                                destination_triangle_point2[1] - y],
    ...
    cv2.fillConvexPoly(cropped_destination_rectangle_mask,
                       destination_triangle_points, 255)

    # for demo, select triangle 10

    # cv2.line(destination_image, destination_triangle_point1,
    #          destination_triangle_point2, (255, 255, 255), 1)
    # cv2.line(destination_image, destination_triangle_point2,
    #          destination_triangle_point3, (255, 255, 255), 1)
    # cv2.line(destination_image, destination_triangle_point3,
    #          destination_triangle_point1, (255, 255, 255), 1)
    ...
    #     cv2.imshow("9.3: Destination filled rectangle mask",
    ...
    Matrix = cv2.getAffineTransform(
        source_triangle_points, destination_triangle_points)
    # creating the warped triangle
    warped_triangle = cv2.warpAffine(cropped_source_rectangle, Matrix, (w, h))
    # for demo, select triangle 10
    # if i == 40:
    #     cv2.imshow(
    #         "10.1: warped source triangle wrt the destination triangle points", warped_triangle)
    # placing destination rectangle mask over the warped triangle
    warped_triangle = cv2.bitwise_and(
        warped_triangle, warped_triangle, mask=cropped_destination_rectangle_mask)
    # for demo, select triangle 10
    # if i == 40:
    #     cv2.imshow("10.2: warped source triangle with the mask",
    #                warped_triangle)
    ...
    new_dest_face_canvas_area_gray = cv2.cvtColor(
        new_dest_face_canvas_area, cv2.COLOR_BGR2GRAY)
    # creating a mask to cut the pixels inside triangle excluding the white lines
    _, mask_created_triangle = cv2.threshold(
        new_dest_face_canvas_area_gray, 1, 255, cv2.THRESH_BINARY_INV)
    # placing the mask created
    wraped_triangle = cv2.bitwise_and(
        warped_triangle, warped_triangle, mask=mask_created_triangle)
    # place the masked triangle inside the small canvas area
    new_dest_face_canvas_area = cv2.add(
        new_dest_face_canvas_area, wraped_triangle)
    ...
    # for demo, select triangle 10
#     if i == 50:
#         cv2.imshow("11: pasting the triangle at destination canvas",
#                    destination_image_canvas)
...
final_destination_face_mask = cv2.fillConvexPoly(
    final_destination_canvas, destination_face_convexhull, 255)
# cv2.imshow("13.2: the final destination face mask",
...
# cv2.imshow("13.3: the inverted final destination face mask",
#           final_destination_face_mask)

# mask destination face
destination_face_masked = cv2.bitwise_and(
    destination_image, destination_image, mask=final_destination_canvas)
#cv2.imshow("13.4: the destination_face_masked", destination_face_masked)

# place new face into destination image
destination_with_face = cv2.add(
    destination_face_masked, destination_image_canvas)
